...
async function getJson(url){
  const response=await fetch(url,{headers:{Accept:'application/vnd.github+json','User-Agent':'noqeri-web'},signal:AbortSignal.timeout(8000)})
  if(!response.ok)throw new Error(`registry fetch ${response.status}`)
  return response.json()
}
async function listFiles(path,base,files){
  const listing=await getJson(`${API}${path}?ref=main`)
  for(const item of listing){
    if(item.type==='dir')await listFiles(item.path,base,files)
    else if(item.type==='file'){
      if(files.length>=100)throw new Error('package file limit exceeded')
      const response=await fetch(item.download_url,{signal:AbortSignal.timeout(8000)})
      if(!response.ok)throw new Error('package file fetch failed')
      const data=Buffer.from(await response.arrayBuffer())
      files.push({name:item.path.slice(base.length+1),data})
    }
  }
}
...
export async function packageArchive(namespace,name,version){
  ...
  const files=[];await listFiles(release.path,release.path,files);files.sort((a,b)=>a.name.localeCompare(b.name))
  let total=0;for(const f of files)total+=f.data.length;if(total>5*1024*1024)throw new Error('package size limit exceeded')
  const value=gzipSync(tar(files),{level:9});cache.set(key,{time:Date.now(),value});return value
}
