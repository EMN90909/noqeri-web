...
export default function App(){
  const location=useLocation(),[compact,setCompact]=useState(false),{reduced,tokens}=useNoqeriMotion()
  useEffect(()=>{const onScroll=()=>setCompact(window.scrollY>18);onScroll();window.addEventListener('scroll',onScroll,{passive:true});return()=>window.removeEventListener('scroll',onScroll)},[])
  return <div className="site-shell">
    ...
    <main id="main" className="route-stage"><motion.div key={location.pathname} className="page-transition premium-route" initial={reduced?false:{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{duration:reduced?0:tokens.duration.fast,ease:tokens.ease.editorial}}><div className="route-rule" aria-hidden="true"/><Routes location={location}>
